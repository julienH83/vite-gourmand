              ...
              {filtered.map(m => (
                <tr key={m.id} style={{ fontWeight: m.is_read ? 'normal' : 'bold', cursor: 'pointer' }} onClick={() => navigate(`/dashboard/messages/${m.id}`)}>
                  ...
                  <td onClick={e => e.stopPropagation()}>
                    ...
                    <button className="btn btn-small btn-danger" style={{ marginLeft: '0.5rem' }} onClick={() => handleDelete(m.id)}>Supprimer</button>
                  </td>
                </tr>
              ))}
            </tbody>
    ...
    <div>
      ...
      <div className="card" style={{ padding: '1.5rem', marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          {message.is_read && message.read_by_first_name && (
            <span className="text-muted" style={{ fontSize: '0.85rem' }}>Lu par {message.read_by_first_name} {message.read_by_last_name}</span>
          )}
        </div>
